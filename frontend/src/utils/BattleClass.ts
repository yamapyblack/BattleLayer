...
export default class BattleClass {
  ...
  getRnadomNumber = (_num: number): BigInt => {
    ...
    const _randomNum = _encoded % BigInt(_num);
  ...
  private _getSkillTarget = (
    ...
    switch (_skillTarget) {
      case SKILL_TARGET.RandomPlayer:
        _isToPlayer = true;
        _unitIndexes = [Number(this.getRnadomNumber(_playerVals.length))];
        break;

      case SKILL_TARGET.RandomEnemy:
        _isToPlayer = false;
        _unitIndexes = [Number(this.getRnadomNumber(_enemyVals.length))];
        break;
